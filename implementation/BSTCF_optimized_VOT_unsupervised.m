...
function [] = BSTCF_optimized_VOT_unsupervised(params,videos,vid)
...
update_interval=2;%多少帧一更新  原先为2帧。
features    = params.t_features;
video_path  = params.video_path;
s_frames    = params.s_frames;
pos         = floor(params.init_pos);
target_sz   = floor(params.wsize);
im1 = imread([video_path '/' s_frames{1}]);
...
%     [g_f,h_f]=ADMM_solve_h(params,use_sz,model_xf,yf,small_filter_sz,w,model_w,frame); % h值
...
%     end
    ...
    %%%
    [g_f,h_f]=ADMM_solve_h(params,use_sz,model_xf,yf,small_filter_sz,w,model_w,frame,h_f3); % h值
    for iteration = 1:params.al_iteration-1
        [w]=ADMM_solve_w(params,use_sz,model_w,h_f);
   ...
   % ADMM solution for scale estimation 尺度预测
%     h_ff1 = h_f(:,:,1:31);
...
%   save resutls.
baselinepath=['E:/BSTCF/unsupervised/',videos{vid}];%results save path
mkdir(baselinepath);
...
dlmwrite([baselinepath '/' videos{vid} '_time.txt'], timeu,'newline','pc', 'precision',5);
end
