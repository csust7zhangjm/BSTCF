          ...
          Geom<T> geom)
{
  ...
  int roi_start_h = (int)::roundf(v1) - 1 ;
  int roi_start_w = (int)::roundf(u1) - 1 ;
  int roi_end_h   = (int)::roundf(v2) - 1 ;
  int roi_end_w   = (int)::roundf(u2) - 1 ;
  ...
  b.wstart = (int)floorf(((T)pw) * bin_size_w) ;
  b.wend = (int)ceilf(((T)(pw + 1)) * bin_size_w) ;
  b.wstart = min(max(b.wstart + roi_start_w, 0), (int)width) ;
  b.wend = min(max(b.wend + roi_start_w, 0), (int)width) ;

  b.hstart = (int)floorf(((T)ph) * bin_size_h) ;
  b.hend = (int)ceilf(((T)(ph + 1)) * bin_size_h) ;
